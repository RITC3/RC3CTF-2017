...
#define STACK_MEMBERS 		     512
#define HEAP_MEMBERS 			     512
...
struct stack_model {
	long long size;
	long long current;
	long long contents[STACK_MEMBERS];
};
typedef struct stack_model stack_model;

struct instruction_model {
	char** unique_id;
	void (**instruction_function)(stack_model * stack, char * parameter, int size);
...
int read_source(char** buffer);
void check_source(char* buffer);
bool initialize_stack(stack_model * stack);
bool stack_push(stack_model * stack, long long val);
long long stack_pop(stack_model * stack);
long long stack_peak(stack_model * stack, int depth);
...
void step_through_program(stack_model * stack, char * source);

// All the functions representing actual whitespace instructions
void ws_stack_push(stack_model * stack, char* parameter, int size);
void ws_stack_dup(stack_model * stack, char* parameter, int size);
void ws_stack_copy(stack_model * stack, char* parameter, int size);
void ws_stack_swap(stack_model * stack, char* parameter, int size);
void ws_stack_discard(stack_model * stack, char* parameter, int size);
void ws_stack_slide(stack_model * stack, char* parameter, int size);
void ws_math_add(stack_model * stack, char* parameter, int size);
void ws_math_sub(stack_model * stack, char* parameter, int size);
void ws_math_mult(stack_model * stack, char* parameter, int size);
void ws_math_div(stack_model * stack, char* parameter, int size);
void ws_math_mod(stack_model * stack, char* parameter, int size);
void ws_heap_store(stack_model * stack, char* parameter, int size);
void ws_heap_retrieve(stack_model * stack, char* parameter, int size);
void ws_flow_mark(stack_model * stack, char* parameter, int size);
void ws_flow_call(stack_model * stack, char* parameter, int size);
void ws_flow_jump(stack_model * stack, char* parameter, int size);
void ws_flow_jz(stack_model * stack, char* parameter, int size);
void ws_flow_jn(stack_model * stack, char* parameter, int size);
void ws_flow_ret(stack_model * stack, char* parameter, int size);
void ws_flow_exit(stack_model * stack, char* parameter, int size);
void ws_io_outc(stack_model * stack, char* parameter, int size);
void ws_io_outn(stack_model * stack, char* parameter, int size);
void ws_io_inc(stack_model * stack, char* parameter, int size);
void ws_io_inn(stack_model * stack, char* parameter, int size);
...
int main(int argc, char** argv)
{
	...
	while(read_source(&source))
	{
		current_instruction_index = 0;
		memset(&instruction_index, 0, sizeof(instruction_index));

		check_source(source);
		initialize_stack(&stack);
		if (!create_heap())
		{
			dprintf("heap creation failed\n");
			exit(1);
		}

		if (!create_label_table()) {
			dprintf("label table creation failed\n");
			exit(1);
		}

		if (!create_instruction_set()) {
			dprintf("instruction set creation failed\n");
			exit(1);
		}

		if (!locate_jump_labels(source)) {
			dprintf("locate jump labs failed\n");
			exit(1);
		}

		step_through_program(&stack, source);
...
int read_source(char** buffer)
{
	size_t size, total_read = 0;
	int num_bytes;

	num_bytes = read(0, &size, sizeof(size));
	//num_bytes = fread(&size, 1, sizeof(size), stdin);
	if(num_bytes != sizeof(size) || size > MAX_SIZE)
	{
		dprintf("Bad size num_bytes = %d, size = %lu\n", num_bytes, size);
		exit(1);
	}
	...
	*buffer = calloc(size+1, 1);
	...
	{
		//num_bytes = fread((*buffer) + total_read, 1, size - total_read, stdin);
		num_bytes = read(0, (*buffer) + total_read, size - total_read);
...
bool initialize_stack(stack_model * stack)
{
	memset(stack->contents, 0, sizeof(stack->contents));
...
bool stack_push(stack_model * stack, long long val)
{
	//Disable the check for underflowing the stack
	stack->current--;
...
long long stack_pop(stack_model * stack)
{
	//Disable the check for overflowing the stack
	stack->current++;
	...
	while (current_instruction_index != -1 && source[current_instruction_index]) {
		for (i = 0; i < MAX_INSTRUCTIONS; i++) {
			if (!strncmp(&(source[current_instruction_index]), instruction_set.unique_id[i], instruction_set.instruction_size[i])) {
				instruction_set.instruction_function[i](stack, &(source[current_instruction_index]), instruction_set.instruction_size[i]);
				break;
			}
		}
...
void ws_stack_push(stack_model * stack, char* parameter, int size)
{
	...
	if (leap = retrieve_label_or_number(&(parameter[size]), &label_number)) {
		stack_push(stack, convert_ws_to_number(label_number));
...
void ws_stack_dup(stack_model * stack, char* parameter, int size)
{
	stack_push(stack, stack_peak(stack, 0));
...
void ws_stack_copy(stack_model * stack, char* parameter, int size)
{
	...
	int leap = 0;
	if (leap = retrieve_label_or_number(&(parameter[size]), &label_number)) {
		stack_push(stack, stack_peak(stack, convert_ws_to_number(label_number)));
		current_instruction_index += size + leap + 1;
		free (label_number);
	}
	else
...
void ws_stack_swap(stack_model * stack, char* parameter, int size)
{
	long long first_off = stack_pop(stack);
	long long second_off = stack_pop(stack);
	stack_push(stack, first_off);
	stack_push(stack, second_off);
	current_instruction_index += size;
	return;
}
...
void ws_stack_slide(stack_model * stack, char* parameter, int size)
{
	long long first = stack_pop(stack);
	int leap = 0;
	char* label_number = NULL;
	if (leap = retrieve_label_or_number(&(parameter[size]), &label_number)) {
...
void ws_heap_store(stack_model * stack, char* parameter, int size)
{
	long long value = stack_pop(stack);
	long long addr = stack_pop(stack);
	heap_put(value, addr);
	current_instruction_index += size;
	return;
}
...
void ws_heap_retrieve(stack_model * stack, char* parameter, int size)
{
	stack_push(stack, heap_get(stack_pop(stack)));
	current_instruction_index += size;
	return;
}
...
void ws_flow_call(stack_model * stack, char* parameter, int size)
{
	...
	if (leap = retrieve_label_or_number(&(parameter[size]), &label)) {
		if (add_ret_addr(current_instruction_index + size + leap + 1)) {
			ws_flow_jump(stack, parameter, size);
		}
		free (label);
	}
	else {
...
void ws_io_inc(stack_model * stack, char* parameter, int size)
{
	...
	heap_put((long long)c, stack_pop(stack));
	current_instruction_index += size;
	return;
}
...
void ws_io_inn(stack_model * stack, char* parameter, int size)
{
	char s[19], * e;
	scanf("%18s", s);
	heap_put(strtoll(s, &e, 10), stack_pop(stack));
	current_instruction_index += size;
	return;
